...
def feature_importances_cli(argvs=sys.argv[1:]):
    parser = argparse.ArgumentParser("Store the feature importances")
    parser.add_argument(
        "-mc",
        "--main_category",
...
def store_feature_importances(main_category, category):
    ...
    data.drop(
        columns=data.columns[
            ...
            | data.columns.str.startswith("residual")
            | data.columns.str.startswith("survival")
            | data.columns.isin([DEATH_COLUMN, FOLLOW_UP_TIME_COLUMN, "fold"])
        ],
        inplace=True,
    )
    data.drop(
    ...
    feature_importances = pd.DataFrame(
        None,
        columns=["feature_importances", "std"],
        index=pd.MultiIndex.from_product(
            (TARGETS, ALGORITHMS, RANDOM_STATES, data.columns),
            names=["target", "algorithm", "random_state", "predictors"],
        ),
    )
            ...
            for random_state in RANDOM_STATES:
                ...
                if all(feasibility):
                    feature_importances.loc[
                        (target, algorithm, random_state, data.columns),
                        "feature_importances",
                    ] = data.loc[
                        f"feature_importances_{target}_{algorithm}_{random_state}_train"
                    ].values
                    feature_importances.loc[
                        (target, algorithm, random_state, data.columns), "std"
                    ] = (
                        data.loc[
                            [
                                f"feature_importances_{target}_{algorithm}_{random_state}_{fold}"
    ...
    feature_importances.reset_index().to_feather(
        f"data/feature_importances/{main_category}/{category}.feather"
    )

...
def get_correlation_all_same(target, main_category, category, algorithm, data, folds, correlations, correlations_std):
    ...
    if f"{residual}_1" in data.columns and f"{residual}_2" in data.columns:
        for method in CORRELATIONS_METHODS:
            ...
            correlations[method].loc[(main_category, category, algorithm), (main_category, category, algorithm)] = data[f"{residual}_1"].corr(data[f"{residual}_2"], method=method)                             
            correlations_std[method].loc[(main_category, category, algorithm), (main_category, category, algorithm)] = pd.Series(list_correlations_std).mean()
...
def get_correlation_same_main_category_category(main_category, category, target_idx, algorithm_idx, target_col, algorithm_col, data_idx, folds, correlations, correlations_std):
    ...
    if f"{residual_idx}_1" in data_idx.columns and f"{residual_idx}_2" in data_idx.columns and f"{residual_col}_1" in data_idx.columns and f"{residual_col}_2" in data_idx.columns:
        for method in CORRELATIONS_METHODS:
            ...
            correlations_std[method].loc[(main_category, category, algorithm_idx), (main_category, category, algorithm_col)] = pd.Series(list_correlations_std).mean()
...
def compute_correlation(target_idx, target_col):
    ...
    number_participants.to_feather(f"data/correlation/residual/number_participants_{target_idx}_{target_col}.feather")
    for method in CORRELATIONS_METHODS:
        correlations[method].to_feather(f"data/correlation/residual/correlations_{target_idx}_{target_col}.feather")    
        correlations_std[method].to_feather(f"data/correlation/residual/correlations_std_{target_idx}_{target_col}.feather")
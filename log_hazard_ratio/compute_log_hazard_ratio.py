...
def log_hazard_ratio_cli(argvs=sys.argv[1:]):
    ...
    parser.add_argument("-sa", "--source_algorithm", help="Algorithm that has been used to compute the prediction from which the residuals have been computes.", required=True)
...
def compute_log_hazard_ratio(main_category, category, source_algorithm):
    for random_state in [1, 2]:
        ...
        data.drop(columns=data.columns[~data.columns.isin([residual_column, AGE_COLUMN, GENDER_COLUMN, FOLLOW_UP_TIME_COLUMN, DEATH_COLUMN] + ETHNICITIES)], inplace=True)
        data.drop(index=data.index[data.index.astype(str).str.startswith("feature_importances") | data[DEATH_COLUMN].isna()], inplace=True)
        ...
        if (not data.empty) and ((data[DEATH_COLUMN] == 1.0).sum() > 0):
            ...
            cph.fit(scaled_data, duration_col=FOLLOW_UP_TIME_COLUMN, event_col=DEATH_COLUMN)

            log_hazard_ratio, std_error, p_value = cph.summary.loc[residual_column, ["coef", "se(coef)", "p"]]

            metrics = {"log hazard ratio": log_hazard_ratio, "std": std_error,  "p-value": p_value}
        else:

...
def compute_log_hazard_ratio(main_category, category, source_algorithm):
    for random_state in [1, 2]:
        ...
        if (not data.empty) and ((data[DEATH_COLUMN] == 1.0).sum() > 0):
            ...
            log_hazard_ratio, p_value, std_error = cph.summary.loc[residual_column, ["coef", "p", "se(coef)"]]

            metrics = {"log hazard ratio": log_hazard_ratio, "p-value": p_value, "std": std_error}
        else:

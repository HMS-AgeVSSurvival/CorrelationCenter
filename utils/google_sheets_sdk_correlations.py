...
def get_spreadsheet():
    ...
    return gc.open_by_key(os.environ.get("GOOGLE_CORRELATIONS_SHEET_ID"))
...
def merge_cols(sheet_name, row, cols):
        ...
        "requests": [
            {
                "mergeCells": {
                    ...
                    "range": {
                        "sheetId": spreadsheet.worksheet(sheet_name)._properties['sheetId'],
                        "startRowIndex": row - 1,
                        "endRowIndex": row,
...
def merge_rows(sheet_name, rows, col):
